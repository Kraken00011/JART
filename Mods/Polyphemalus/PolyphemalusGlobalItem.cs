...
public class PolyphemalusGlobalItem : GlobalItem
{
    ...
    {
        return Language.ActiveCulture.Name == "ru-RU" && ModInstances.Polyphemalus != null && JARTLocalizationConf.Instance.PolyphemalusLocalization;
    }
    public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
    {
        if (item.type == ModContent.ItemType<PolypebralShield>())
        {
            ItemHelper.TranslateTooltip(tooltips, l => l.Mod == "Terraria" && l.Name == "Tooltip0", tooltip =>
            {
                ...
                {
                    tooltip.Text = $"Нажмите {parts[1]}, чтобы совершить всесторонний рывок, наносящий урон\nРывок имеет долгую перезарядку\nПопытка совершения ещё одного рывка в момент попадания по врагу позволит вам управлять отскоком от него";
                }

                else
                    tooltip.Text = $"Нажмите [Не назначено], чтобы совершить всесторонний рывок, наносящий урон\nРывок имеет долгую перезарядку\nПопытка совершения ещё одного рывка в момент попадания по врагу позволит вам управлять отскоком от него";
            });
        }
    }
